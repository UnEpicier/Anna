...
async function getData() {
	...
	const [servicesRes, departmentsRes, informationsRes, schedulesRes] =
		await Promise.all([
			...
			fetch(`${process.env.API_URL}/departments/actives`, { headers: authHeaders }),
			fetch(`${process.env.API_URL}/informations`, { headers: authHeaders }),
	...
	const [services, departments, informations, schedules] = await Promise.all([
		...
		departmentsRes.json() as Promise<ResponseObject<{ id: string; name: string }[]>>,
	...
	return {
		...
		departments: departments.success ? departments.responseObject.length : 0,
