		...
		>
			...
			<div className='border border-sky-200 bg-sky-50 p-4 flex gap-3'>
				...
				<div>
					...
					<p className='text-sm text-sky-800/80'>
						Sélectionnez les départements où vous intervenez. Ils
						seront affichés en surbrillance sur la carte de la page
						contact.
					</p>
				</div>
			</div>
						...
						{filteredDepartments.map((dept) => {
							...
							return (
								...
								>
									...
									<span className='text-current/70 ml-2'>
										{dept.name}
									</span>
								</button>
							);
			...
			<div className='pt-2 border-t border-border flex items-center justify-between'>
				<span className='text-xs text-muted-foreground'>
					{selectedDepartments.length} département
					{selectedDepartments.length > 1 ? 's' : ''} sélectionné
