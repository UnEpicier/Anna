...
describe("Departments API Endpoints", () => {
	...
	describe("PUT /departments/:id", () => {
		it("should update department active status", async () => {
			...
			expect(response.statusCode).toEqual(StatusCodes.OK);
			expect(responseBody.success).toBeTruthy();
			expect(responseBody.message).toContain("Department updated successfully");
			expect(responseBody.responseObject).toBeDefined();
			if (responseBody.responseObject) {
				expect(responseBody.responseObject.active).toEqual(updatedActiveStatus);
				compareDepartment(
...
function compareDepartment(mockDepartment: Department, responseDepartment: Department) {
	if (!mockDepartment || !responseDepartment) {
		throw new Error("Invalid test data: mockDepartment or responseDepartment is undefined");
	}
