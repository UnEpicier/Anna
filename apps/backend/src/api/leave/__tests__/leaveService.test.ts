...
describe("leaveService", () => {
	...
	describe("find", () => {
		it("return leave", async () => {
			// Arrange
			(leaveRepositoryInstance.findAsync as Mock).mockReturnValue(mockLeave);
		...
		it("handles errors for findAsync", async () => {
			// Arrange
			(leaveRepositoryInstance.findAsync as Mock).mockRejectedValue(new Error("Database error"));
			...
			expect(result.success).toBeFalsy();
			expect(result.message).equals("An error occurred while retrieving leave.");
			expect(result.responseObject).toBeNull();
		});
	});
	...
	describe("create", () => {
		...
		it("handles errors for createAsync", async () => {
			...
			(leaveRepositoryInstance.findAsync as Mock).mockReturnValue(null);
			(leaveRepositoryInstance.createAsync as Mock).mockRejectedValue(new Error("Database error"));
			...
			expect(result.statusCode).toEqual(StatusCodes.INTERNAL_SERVER_ERROR);
			expect(result.success).toBeFalsy();
			expect(result.message).toEqual("An error occurred while creating leave.");
			expect(result.responseObject).toBeNull();
		});
	});
	...
	describe("update", () => {
		it("updates and returns the leave", async () => {
			...
			(leaveRepositoryInstance.findAsync as Mock).mockReturnValue(mockLeave);
			(leaveRepositoryInstance.updateAsync as Mock).mockReturnValue(updatedInformations);
		...
		it("handles errors for updateAsync", async () => {
			...
			const updateData: Partial<Leave> = { to: new Date("2024-12-31") };
			(leaveRepositoryInstance.findAsync as Mock).mockReturnValue(mockLeave);
			(leaveRepositoryInstance.updateAsync as Mock).mockRejectedValue(new Error("Database error"));
			...
			expect(result.statusCode).toEqual(StatusCodes.INTERNAL_SERVER_ERROR);
			expect(result.success).toBeFalsy();
			expect(result.message).toEqual("An error occurred while updating leave.");
			expect(result.responseObject).toBeNull();
		});
	});
	...
	describe("delete", () => {
		...
		it("handles errors for deleteAsync", async () => {
			// Arrange
			(leaveRepositoryInstance.findAsync as Mock).mockReturnValue(mockLeave);
			(leaveRepositoryInstance.deleteAsync as Mock).mockRejectedValue(new Error("Database error"));
			...
			expect(result.statusCode).toEqual(StatusCodes.INTERNAL_SERVER_ERROR);
			expect(result.success).toBeFalsy();
			expect(result.message).toEqual("An error occurred while deleting leave.");
			expect(result.responseObject).toBeNull();
		});
	});
});
