...
export class LeaveService {
	...
	async find(): Promise<ServiceResponse<Leave | null>> {
		try {
			...
			if (!leave) {
				return ServiceResponse.failure("No leave found", null, StatusCodes.NO_CONTENT);
			}
	...
	async create(data: Omit<Leave, "id" | "createdAt" | "updatedAt">): Promise<ServiceResponse<Leave | null>> {
		try {
			const leaveExists = await this.leaveRepository.findAsync();
			if (leaveExists) {
				return ServiceResponse.failure("Leave already exists", null, StatusCodes.CONFLICT);
			}

			const newLeave = await this.leaveRepository.createAsync(data);
			return ServiceResponse.success<Leave>("Leave created successfully", newLeave, StatusCodes.CREATED);
		} catch (error) {
	...
	async update(data: Partial<Leave>): Promise<ServiceResponse<Leave | null>> {
		try {
			...
			return ServiceResponse.success<Leave>("Leave updated successfully", updatedLeave);
		} catch (error) {
	...
	async delete(): Promise<ServiceResponse<Leave | null>> {
		try {
			...
			return ServiceResponse.success<null>("Leave deleted successfully", null);
		} catch (error) {
