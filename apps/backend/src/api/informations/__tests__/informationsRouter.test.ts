	...
	describe('PUT /informations', () => {
		it('should update and return the updated informations object', async () => {
			...
			if (responseBody.responseObject) {
				expect(responseBody.responseObject.phone).toEqual(
					updateData.phone
				);
				expect(responseBody.responseObject.address).toEqual(
					updateData.address
				);
				compareInformations(
...
function compareInformations(
	mockInformations: Informations,
	responseInformations: Informations
) {
	if (!mockInformations || !responseInformations) {
		throw new Error(
			'Invalid test data: mockInformations or responseInformations is undefined'
		);
	}
