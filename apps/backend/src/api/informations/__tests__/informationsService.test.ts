...
describe("informationsService", () => {
	...
	beforeEach(() => {
		...
		informationsServiceInstance = new InformationsService(informationsRepositoryInstance);
	});

	describe("findAll", () => {
		it("return informations", async () => {
			// Arrange
			(informationsRepositoryInstance.findAsync as Mock).mockReturnValue(mockInformations);
		...
		it("returns a not found error for no informations found", async () => {
			// Arrange
			(informationsRepositoryInstance.findAsync as Mock).mockReturnValue(null);
		...
		it("handles errors for findAsync", async () => {
			// Arrange
			(informationsRepositoryInstance.findAsync as Mock).mockRejectedValue(new Error("Database error"));
			...
			expect(result.success).toBeFalsy();
			expect(result.message).equals("An error occurred while retrieving informations.");
			expect(result.responseObject).toBeNull();
		});
	});
	...
	describe("update", () => {
		it("updates and returns the informations", async () => {
			...
			const updatedInformations: Informations = { ...mockInformations, ...updateData };
			(informationsRepositoryInstance.updateAsync as Mock).mockReturnValue(updatedInformations);
		...
		it("handles errors for updateAsync", async () => {
			// Arrange
			const updateData: Partial<Informations> = { phone: "0987654321" };
			(informationsRepositoryInstance.updateAsync as Mock).mockRejectedValue(new Error("Database error"));
			...
			expect(result.statusCode).toEqual(StatusCodes.INTERNAL_SERVER_ERROR);
			expect(result.success).toBeFalsy();
			expect(result.message).toEqual("An error occurred while updating informations.");
			expect(result.responseObject).toBeNull();
		});
	});
});
