...
export class InformationsService {
	...
	constructor(repository: InformationsRepository = new InformationsRepository()) {
	...
	async find(): Promise<ServiceResponse<Informations | null>> {
		try {
			...
			if (!informations) {
				return ServiceResponse.failure("No Informations found", null, StatusCodes.NOT_FOUND);
			}
			return ServiceResponse.success<Informations>("Informations found", informations);
		} catch (error) {
	...
	async update(data: Partial<Informations>): Promise<ServiceResponse<Informations | null>> {
		try {
			const updatedInformations = await this.informationsRepository.updateAsync(data);
			return ServiceResponse.success<Informations>("Informations updated successfully", updatedInformations);
		} catch (error) {
