...
describe("Auth API Endpoints", () => {
	describe("POST /auth/login", () => {
		it("should return 401 for invalid credentials", async () => {
			// Act
			const response = await request(app).post("/auth/login").send({ email: "invalidmail@example.com" });
	...
	describe("POST /auth/verify-code", () => {
		it("should return 401 for invalid authentication code", async () => {
			...
			expect(response.statusCode).toEqual(401);
			expect(responseBody.success).toBeFalsy();
			expect(responseBody.message).toContain("Authentication code expired or not found");
		});
	});
});
