...
export class AuthService {
	...
	async login(email: string): Promise<ServiceResponse<string | null>> {
		try {
			...
			redisClient.setEx(`authCode:${sessionToken}`, 10 * 60, await bcrypt.hash(authCode, this.salt));

			return ServiceResponse.success<string>("Login initiated", sessionToken, StatusCodes.OK);
		} catch (error) {
	...
	async verifyCode(sessionToken: string, code: string): Promise<ServiceResponse<string | null>> {
		try {
			const storedCode = await redisClient.get(`authCode:${sessionToken}`);

			if (!storedCode) {
				return ServiceResponse.failure("Authentication code expired or not found", null, StatusCodes.UNAUTHORIZED);
			}

			if (!(await bcrypt.compare(code, storedCode))) {
				return ServiceResponse.failure("Invalid authentication code", null, StatusCodes.UNAUTHORIZED);
			}

			redisClient.del(`authCode:${sessionToken}`);
			redisClient.setEx(`loginSession:${sessionToken}`, 4 * 60 * 60, sessionToken);

			return ServiceResponse.success<string>("Authentication successful", sessionToken, StatusCodes.OK);
		} catch (error) {
