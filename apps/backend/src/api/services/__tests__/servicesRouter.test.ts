...
describe("Services API Endpoints", () => {
	...
	describe("POST /services", () => {
		it("should create a new service", async () => {
			...
			expect(responseBody.success).toBeTruthy();
			expect(responseBody.message).toContain("Service created successfully");
	...
	describe("PUT /services/:id", () => {
		it("should update an existing service", async () => {
			...
			// Act
			const response = await request(app).put(`/services/${service.id}`).send(updatedServiceData);
			const responseBody: ServiceResponse<Service> = response.body;

			// Assert
			expect(response.statusCode).toEqual(StatusCodes.OK);
			expect(responseBody.success).toBeTruthy();
			expect(responseBody.message).toContain("Service updated successfully");
			expect(responseBody.responseObject.title).toEqual(updatedServiceData.title);
		});
		...
		it("should return 400 for invalid id", async () => {
			...
			// Act
			const response = await request(app).put("/services/invalid-id").send(updatedServiceData);
			const responseBody: ServiceResponse = response.body;
...
function compareService(mockService: Service, responseService: Service) {
	if (!mockService || !responseService) {
		throw new Error("Invalid test data: mockService or responseService is undefined");
	}
