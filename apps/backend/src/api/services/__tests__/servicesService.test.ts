...
describe("servicesService", () => {
	...
	describe("find", () => {
		it("returns the service", async () => {
			// Arrange
			(servicesRepositoryInstance.findById as Mock).mockReturnValue(mockService);

			// Act
			const result = await servicesServiceInstance.find(`${mockService.id}`);
		...
		it("handles errors for find", async () => {
			// Arrange
			(servicesRepositoryInstance.findById as Mock).mockRejectedValue(new Error("Database error"));
			...
			expect(result.statusCode).toEqual(StatusCodes.INTERNAL_SERVER_ERROR);
			expect(result.success).toBeFalsy();
			expect(result.message).toEqual("An error occurred while retrieving service.");
			expect(result.responseObject).toBeNull();
		});
	});
	...
	describe("create", () => {
		it("creates and returns the service", async () => {
			...
			const createdService: Service = { id: 2, ...serviceData, createdAt: new Date(), updatedAt: new Date() };
			(servicesRepositoryInstance.create as Mock).mockReturnValue(createdService);
		...
		it("handles errors for create", async () => {
			...
			(servicesRepositoryInstance.create as Mock).mockRejectedValue(new Error("Database error"));
			...
			expect(result.statusCode).toEqual(StatusCodes.INTERNAL_SERVER_ERROR);
			expect(result.success).toBeFalsy();
			expect(result.message).toEqual("An error occurred while creating service.");
			expect(result.responseObject).toBeNull();
		});
	});
	...
	describe("update", () => {
		it("updates and returns the service", async () => {
			...
			(servicesRepositoryInstance.update as Mock).mockReturnValue(updatedService);

			// Act
			const result = await servicesServiceInstance.update(`${mockService.id}`, updateData);
		...
		it("handles errors for update", async () => {
			// Arrange
			const updateData: Partial<Service> = { enabled: false };
			(servicesRepositoryInstance.update as Mock).mockRejectedValue(new Error("Database error"));

			// Act
			const result = await servicesServiceInstance.update("1", updateData);

			// Assert
			expect(result.statusCode).toEqual(StatusCodes.INTERNAL_SERVER_ERROR);
			expect(result.success).toBeFalsy();
			expect(result.message).toEqual("An error occurred while updating service.");
			expect(result.responseObject).toBeNull();
		});
	});
