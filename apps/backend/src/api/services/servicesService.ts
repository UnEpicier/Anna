...
export class ServicesService {
	...
	async findAll(): Promise<ServiceResponse<Service[] | null>> {
		try {
			...
			if (!services) {
				return ServiceResponse.failure("Any services found", null, StatusCodes.NOT_FOUND);
			}
	...
	async create(
		...
		try {
			...
			return ServiceResponse.success<Service>("Service created successfully", newService, StatusCodes.CREATED);
		} catch (error) {
	...
	async update(id: string, data: Partial<Service>): Promise<ServiceResponse<Service | null>> {
		try {
			const parsedId = parseInt(id, 10);
			if (!parsedId || parsedId <= 0) {
				return ServiceResponse.failure("Invalid ID provided", null, StatusCodes.BAD_REQUEST);
			...
			const updatedInformations = await this.servicesRepository.update(parsedId, data);
			return ServiceResponse.success<Service>("Service updated successfully", updatedInformations);
		} catch (error) {
	...
	async delete(id: string): Promise<ServiceResponse> {
		try {
			...
			return ServiceResponse.success<null>("Service deleted successfully", null);
		} catch (error) {
