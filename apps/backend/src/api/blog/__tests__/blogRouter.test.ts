...
describe("Blog API Endpoints", () => {
	...
	describe("GET /blog/categories/:id", () => {
		it("should return a service object", async () => {
			// Act
			const response = await request(app).get(`/blog/categories/${categories[0].id}`);
			const responseBody: ServiceResponse<BlogCategory> = response.body;
		...
		it("should return 400 for invalid id", async () => {
			// Act
			const response = await request(app).get("/blog/categories/invalid-id");
	...
	describe("GET /blog/posts/:url", () => {
		it("should return a service object", async () => {
			// Act
			const response = await request(app).get(`/blog/posts/${posts[0].uri}`);
			const responseBody: ServiceResponse<BlogPost> = response.body;
	...
	describe("POST /blog/categories", () => {
		it("should create a new category", async () => {
			...
			// Act
			const response = await request(app).post("/blog/categories").send(newCategory);
			const responseBody: ServiceResponse<BlogCategory> = response.body;

			// Assert
			expect(response.statusCode).toEqual(StatusCodes.CREATED);
			expect(responseBody.success).toBeTruthy();
			expect(responseBody.message).toContain("Blog category created successfully");
		...
		it("should return 400 for missing required fields", async () => {
			...
			// Act
			const response = await request(app).post("/blog/categories").send(invalidCategory);
			const responseBody: ServiceResponse = response.body;
		...
		it("should return 409 for existing category", async () => {
			...
			// Act
			const response = await request(app).post("/blog/categories").send(existingCategory);
			const responseBody: ServiceResponse = response.body;

			// Assert
			expect(response.statusCode).toEqual(StatusCodes.CONFLICT);
			expect(responseBody.success).toBeFalsy();
			expect(responseBody.message).toContain("Blog category already exists");
			expect(responseBody.responseObject).toBeNull();
		});
	});
	...
	describe("POST /blog/posts", () => {
		it("should create a new post", async () => {
			...
			expect(response.statusCode).toEqual(StatusCodes.CREATED);
			expect(responseBody.success).toBeTruthy();
			expect(responseBody.message).toContain("Blog post created successfully");
		...
		it("should return 400 for missing fields", async () => {
			...
			// Act
			const response = await request(app).post("/blog/posts").send(invalidPost);
			const responseBody: ServiceResponse = response.body;
		...
		it("should return 409 for existing post title", async () => {
			// Arrange
			const existingPost: Omit<BlogPost, "id" | "createdAt" | "updatedAt"> = {
				title: posts[0].title,
				uri: "unique-post-url",
			...
			// Act
			const response = await request(app).post("/blog/posts").send(existingPost);
			const responseBody: ServiceResponse = response.body;

			// Assert
			expect(response.statusCode).toEqual(StatusCodes.CONFLICT);
			expect(responseBody.success).toBeFalsy();
			expect(responseBody.message).toContain("Blog post with the same title already exists");
			expect(responseBody.responseObject).toBeNull();
		});
		...
		it("should return 409 for existing post URL", async () => {
			...
			expect(response.statusCode).toEqual(StatusCodes.CONFLICT);
			expect(responseBody.success).toBeFalsy();
			expect(responseBody.message).toContain("Blog post with the same uri already exists");
			expect(responseBody.responseObject).toBeNull();
		});
	});
	...
	describe("PUT /blog/categories/:id", () => {
		it("should update an existing category", async () => {
			...
			// Act
			const response = await request(app).put(`/blog/categories/${categories[0].id}`).send(updatedCategory);
			const responseBody: ServiceResponse<BlogCategory> = response.body;

			// Assert
			expect(response.statusCode).toEqual(StatusCodes.OK);
			expect(responseBody.success).toBeTruthy();
			expect(responseBody.message).toContain("Blog category updated successfully");
			expect(responseBody.responseObject.name).toEqual(updatedCategory.name);
		});
		...
		it("should return 400 for invalid id", async () => {
			...
			// Act
			const response = await request(app).put("/blog/categories/invalid-id").send(updatedCategory);
			const responseBody: ServiceResponse = response.body;
		...
		it("should return 404 for non-existing category", async () => {
			...
			// Act
			const response = await request(app).put("/blog/categories/9999").send(updatedCategory);
			const responseBody: ServiceResponse = response.body;

			// Assert
			expect(response.statusCode).toEqual(StatusCodes.NOT_FOUND);
			expect(responseBody.success).toBeFalsy();
			expect(responseBody.message).toContain("No blog category found to update");
			expect(responseBody.responseObject).toBeNull();
		});
	});
	...
	describe("PUT /blog/posts/:id", () => {
		it("should update an existing post", async () => {
			...
			// Act
			const response = await request(app).put(`/blog/posts/${posts[0].id}`).send(updatedPost);
			const responseBody: ServiceResponse<BlogPost> = response.body;

			// Assert
			expect(response.statusCode).toEqual(StatusCodes.OK);
			expect(responseBody.success).toBeTruthy();
			expect(responseBody.message).toContain("Blog post updated successfully");
			expect(responseBody.responseObject.title).toEqual(updatedPost.title);
		});
		...
		it("should return 400 for invalid id", async () => {
			...
			// Act
			const response = await request(app).put("/blog/posts/invalid-id").send(updatedPost);
			const responseBody: ServiceResponse = response.body;
		...
		it("should return 404 for non-existing post", async () => {
			...
			// Act
			const response = await request(app).put("/blog/posts/9999").send(updatedPost);
			const responseBody: ServiceResponse = response.body;

			// Assert
			expect(response.statusCode).toEqual(StatusCodes.NOT_FOUND);
			expect(responseBody.success).toBeFalsy();
			expect(responseBody.message).toContain("No blog post found to update");
			expect(responseBody.responseObject).toBeNull();
		});
	});
	...
	describe("DELETE /blog/categories/:id", () => {
		it("should delete an existing category", async () => {
			// Act
			const response = await request(app).delete(`/blog/categories/${categories[0].id}`);
			const responseBody: ServiceResponse = response.body;

			// Assert
			expect(response.statusCode).toEqual(StatusCodes.OK);
			expect(responseBody.success).toBeTruthy();
			expect(responseBody.message).toContain("Blog category deleted successfully");
		});
		
		it("should return 400 for invalid id", async () => {
			// Act
			const response = await request(app).delete("/blog/categories/invalid-id");
			const responseBody: ServiceResponse = response.body;
		...
		it("should return 404 for non-existing category", async () => {
			...
			expect(response.statusCode).toEqual(StatusCodes.NOT_FOUND);
			expect(responseBody.success).toBeFalsy();
			expect(responseBody.message).toContain("No blog category found to delete");
			expect(responseBody.responseObject).toBeNull();
		});
	});
	...
	describe("DELETE /blog/posts/:id", () => {
		it("should delete an existing post", async () => {
			// Act
			const response = await request(app).delete(`/blog/posts/${posts[0].id}`);
			const responseBody: ServiceResponse = response.body;

			// Assert
			expect(response.statusCode).toEqual(StatusCodes.OK);
			expect(responseBody.success).toBeTruthy();
			expect(responseBody.message).toContain("Blog post deleted successfully");
		});
		
		it("should return 400 for invalid id", async () => {
			// Act
			const response = await request(app).delete("/blog/posts/invalid-id");
			const responseBody: ServiceResponse = response.body;
		...
		it("should return 404 for non-existing post", async () => {
			...
			expect(response.statusCode).toEqual(StatusCodes.NOT_FOUND);
			expect(responseBody.success).toBeFalsy();
			expect(responseBody.message).toContain("No blog post found to delete");
			expect(responseBody.responseObject).toBeNull();
		});
	});
});

function compareCategory(mockCategory: BlogCategory, responseCategory: BlogCategory) {
	if (!mockCategory || !responseCategory) {
		throw new Error("Invalid test data: mockCategory or responseCategory is undefined");
	}
